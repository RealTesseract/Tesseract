...
use WingProxy\event\player\PlayerConnectEvent;
...
class Player extends PMPlayer{
	...
	public function handleLoginPacket(PlayerLoginPacket $packet){
		...
		$pk = WingProxy::getInstance()->getPacket($packet->cachedLoginPacket);
		$pk->decode();
	...
	public function transfer(string $hash){
		$clients = WingProxy::getInstance()->getClientData();
		if(isset($clients[$hash])){
			...
			$pk->uuid = $this->uuid;
			$pk->clientHash = $hash;
			WingProxy::getInstance()->sendDataPacket($pk);

			/*$ip = $clients[$hash]["ip"];
			$port = $clients[$hash]["port"];

			$this->close("", "Transferred to $ip:$port");
			WingProxy::getInstance()->removePlayer($this);*/
		}
	}
