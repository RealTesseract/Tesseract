...
class WingProxy{
	...
	public function __construct(Server $server, array $config){
		...
		$this->interface = new WingProxyInterface($this, $this->serverIp, $this->port);
	...
	public function shutdown(){
		if($this->verified){
			...
			$this->getLogger()->debug("WingProxy client has disconnected from WingProxy server");
		}
	}
			...
			case Info::DISCONNECT_PACKET:
				...
				switch($pk->type){
					case DisconnectPacket::TYPE_GENERIC:
						$this->getLogger()->notice("WingProxy Client has disconnected due to " . $pk->message);
						$this->interface->reconnect();
						break;
